...
module.exports = {
    ...
    getTPDsByWildcard: function (search, listOnly) {
        let promise;
        let AWSregex = new RegExp('.*' + escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(escapeRegExp(search)))))))))))) + '$');
        if (listOnly) {
            promise = tpdModel.find({
